...
Neural training script for the ZHE classifier in the part of Stage II of the project (Stage I: corpus data collection + splitting, CI, Trasition Matrices & Correspondance Analysis; Stage II: Neural Network trainings).
The main stage trains on the corpus and silver labels about the targeted temporal-aspetucal marker ZHE from Stage I and then evaluates / calibrates on the gold set, including silver+gold hybrid runs.
It implements two settings, M0 (context only) and M1 (context + layer features), using a CharCNN–BiLSTM–attention classifier.
Paths, splits and silver/gold mixing thresholds are configured inside this file; run with `python nn_training_(1).py` after preparing the data as described in the project README.
